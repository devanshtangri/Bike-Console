...
sessions_path = ROOT / "lib" / "screens" / "sessions_screen.dart"
detail_path = ROOT / "lib" / "screens" / "session_detail_screen.dart"

if not sessions_path.exists():
    raise SystemExit("Could not find lib/screens/sessions_screen.dart")

sessions = sessions_path.read_text(encoding="utf-8")

# Add the new screen import.
if "import 'session_detail_screen.dart';" not in sessions:
    needle = "import '../services/ride_history_service.dart';\n"
    if needle not in sessions:
        raise SystemExit("Patch failed: could not find sessions_screen import anchor")
    sessions = sessions.replace(
        needle,
        needle + "import 'session_detail_screen.dart';\n",
        1,
    )

old_show_details = """  void _showSessionDetails(SavedRideSession session) {
...
"""
new_show_details = """  void _showSessionDetails(SavedRideSession session) {
...
"""
if old_show_details not in sessions:
    raise SystemExit("Patch failed: could not find _showSessionDetails bottom sheet block")
sessions = sessions.replace(old_show_details, new_show_details, 1)

# Remove the old bottom-sheet detail classes so analyzer does not report unused private elements.
start = sessions.find("class _SessionDetailsSheet extends StatelessWidget {")
end = sessions.find("class _SessionSummary {", start)
if start == -1 or end == -1:
    raise SystemExit("Patch failed: could not find old session details sheet block")
sessions = sessions[:start] + sessions[end:]

sessions_path.write_text(sessions, encoding="utf-8")

# Add the full route detail screen.
detail_code = r'''import 'dart:async';
...
detail_path.write_text(detail_code, encoding="utf-8")

print("Batch 7A session route detail patch applied.")
