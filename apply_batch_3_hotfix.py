...
# Batch 3 made the auto-pause suppression snapshot-backed, but one connection-loss
# path still cleared the guard. When Flutter UI is removed, BLE/plugin detach can
# temporarily report disconnected, which erased the manual-resume guard. Preserve it
# across connection loss; only real movement/start/pause/stop should clear it.
connection_loss_clear_block = '''      _lastIndicatorCommandEpochMs = null;
      _state = _state.copyWith(
        autoPauseSuppressedUntilMovement: false,
        currentRpm: 0,'''
connection_loss_preserve_block = '''      _lastIndicatorCommandEpochMs = null;
      _state = _state.copyWith(
        currentRpm: 0,'''
replace_exact(
    'lib/controllers/ride_session_controller.dart',
    connection_loss_clear_block,
    connection_loss_preserve_block,
)

print('Batch 3 hotfix applied successfully.')
