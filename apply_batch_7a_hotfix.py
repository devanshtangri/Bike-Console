...
ROOT = Path.cwd()
scan_path = ROOT / "lib" / "screens" / "scan_for_devices_screen.dart"
detail_path = ROOT / "lib" / "screens" / "session_detail_screen.dart"

if not scan_path.exists():
    raise SystemExit("Could not find lib/screens/scan_for_devices_screen.dart")
if not detail_path.exists():
    raise SystemExit("Could not find lib/screens/session_detail_screen.dart. Apply Batch 7A first.")

# Fix deprecated GoogleMapController.setMapStyle usage in the new session detail screen.
detail = detail_path.read_text(encoding="utf-8")
detail = detail.replace("    controller.setMapStyle(darkMapStyle);\n\n", "")
if "style: darkMapStyle," not in detail:
...
# Replace the scan screen with a more robust themed scanner.
# Improvements:
# - uses the same matching rules as the BLE connection controller: name/service UUID
# - does not clear the visible device list on every rescan, preventing blank rescan regressions
# - updates results by remoteId so RSSI refreshes in-place as scan packets arrive
# - removes androidUsesFineLocation:false so scan behavior matches the connection controller
scan_code = r'''import 'dart:async';
...
scan_path.write_text(scan_code, encoding="utf-8")

print("Batch 7A hotfix applied. Run: flutter analyze")
